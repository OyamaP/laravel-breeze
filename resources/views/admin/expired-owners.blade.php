                                            ...
                                            <tr>
                                                ...
                                                <td class="px-4 py-3">
                                                    <form id="delete_{{ $owner->id }}" method="post" action="{{ route('admin.expired-owners.destroy', ['owner' => $owner->id]) }}">
                                                        @csrf
                                                        <button type="button" onclick="deletePost(this)" data-id="{{ $owner->id }}" class="text-white bg-red-400 border-0 py-2 px-4 focus:outline-none hover:bg-red-500 rounded">Completely deleted</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
